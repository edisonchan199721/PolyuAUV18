...
//#define p_sen_pin A7  //analog pressure
#define V_BLDC_1_pin 2
#define V_BLDC_2_pin 3
#define BUO_1_pin 5   //for roll
#define BUO_2_pin 6
...
float getdep() {
  ...
  depth = (pressure - initialpressure) * 10 / 9.8099;       //9.8099 in Hong Kong, 9.7807 in singapore
...
void setup() {
  ...
  //pinMode(p_sen_pin, INPUT);
  //error = (analogRead(p_sen_pin) / 1023.0) - 0.04;


  //pinMode(BUO_1_pin, OUTPUT);
  //pinMode(BUO_2_pin, OUTPUT);
  //digitalWrite(BUO_1_pin, LOW);
  //digitalWrite(BUO_2_pin, LOW);
  ...
  pitch_setPt = 0;
  roll_setPt = 0;
  depth_PID.SetOutputLimits(-100, 100);
...
void loop() {
  ...
  /*  if (s_i == 2) {
      ...
      } else if (s_temp[0] == 0xD1) {
        pitch_PID.SetTunings(s_temp[1], s_temp[2], s_temp[3]);
        for (byte i = 3; i < 6; i++)
          EEPROM.put(i * 4, s_temp[i - 2]);
      }
    }*/
  if (pr_i == 4) {
    ...
    //float depth_cm = (p_sen_Val / 1023.0 - error - 0.04) / 0.000369;
    float depth_cm = getdep(); //I2C pressure
    ...
    if (pr[1] > 6)
      digitalWrite(BUO_1_pin, HIGH);
    else if (pr[1] < -6)
      digitalWrite(BUO_2_pin, HIGH);
    else {
      digitalWrite(BUO_1_pin, LOW);
      digitalWrite(BUO_2_pin, LOW);
    }
